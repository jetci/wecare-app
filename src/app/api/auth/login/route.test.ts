...
describe('POST /api/auth/login', () => {
  ...
  it('should login successfully with correct credentials', async () => {
    ...
    expect(responseBody.accessToken).toBe('mocked.access.token');
    expect(responseBody.refreshToken).toBe('mocked.refresh.token');
  ...
  it('should return 401 for incorrect password', async () => {
    ...
    expect(responseBody.error).toBe('รหัสบัตรประชาชนหรือรหัสผ่านไม่ถูกต้อง');
  });
  ...
  it('should return 403 if user is not approved', async () => {
    ...
    expect(responseBody.success).toBe(false);
    expect(responseBody.error).toBe('บัญชีของคุณยังรอการอนุมัติจากผู้ดูแลระบบ');
  });
