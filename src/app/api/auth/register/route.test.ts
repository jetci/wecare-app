...
describe('POST /api/auth/register', () => {
  ...
  it('should return 409 if nationalId already exists', async () => {
    ...
    expect(responseBody.error).toBe('nationalId exists');
  ...
  it('should return 400 for invalid nationalId format (Zod validation)', async () => {
    ...
    expect(responseBody.success).toBe(false);
    expect(responseBody.error.fieldErrors.nationalId).toContain('เลขบัตรประชาชนต้องเป็นตัวเลข 13 หลัก');
  });
  ...
  it('should return 400 for password too short (Zod validation)', async () => {
    ...
    expect(response.status).toBe(400);
    expect(responseBody.success).toBe(false);
    expect(responseBody.error.fieldErrors.password).toContain('String must contain at least 8 character(s)'); // Default Zod message
  });
  ...
  it('should return 400 for invalid phone format (Zod validation)', async () => {
    ...
    expect(response.status).toBe(400);
    expect(responseBody.success).toBe(false);
    expect(responseBody.error.fieldErrors.phone).toContain('เบอร์โทรศัพท์ต้องเป็นตัวเลขเท่านั้น');
  });
   ...
   it('should return 400 for missing required field (e.g., firstName)', async () => {
    ...
    expect(response.status).toBe(400);
    expect(responseBody.success).toBe(false);
    expect(responseBody.error.fieldErrors.firstName).toContain('Required'); // Default Zod message for missing required field
  });
  ...
  it('should return 400 for missing role', async () => {
    ...
    expect(response.status).toBe(400);
    expect(responseBody.success).toBe(false);
    expect(responseBody.error.fieldErrors.role).toContain('Required');
  });
  ...
  it('should handle unexpected errors during registration', async () => {
    ...
    expect(responseBody.success).toBe(false);
    expect(responseBody.error).toBe('Database connection error');
  });
});
