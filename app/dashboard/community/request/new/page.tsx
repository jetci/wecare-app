    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div>
        ...
        <div className="flex gap-2">
          <input type="text" {...register('nationalId')} className="border p-2 flex-1" />
          <button type="button" onClick={handleSearch} disabled={searching} className="px-4 bg-blue-600 text-white">
      ...
      <div>
        ...
        <input type="date" {...register('serviceDate')} min={today} className="border p-2 w-full" />
      ...
      <div>
        ...
        <textarea {...register('details')} className="border p-2 w-full" rows={4} />
        {errors.details && <p className="text-red-500">{errors.details.message}</p>}
      </div>

      <button type="submit" disabled={isSubmitting || !patient} className="px-6 py-2 bg-green-600 text-white rounded disabled:opacity-50">
        ส่งคำขอ
      </button>
    </form>
  );
